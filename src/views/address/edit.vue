<template>
  <div class="body_content">
    <!-- <div class="seach" @click="tosearch()">
      <el-input placeholder="查询收货地址" prefix-icon="el-icon-search" v-model="value"></el-input>
    </div> -->
    <div class="adress">
    ...
    <div class="confirm_add" @click="upd_address">确认修改</div>
    <div class="confirm_del" @click="del_address">删除地址</div>
    ...
    methods: {
      ...
      loadList() {
        address_info({
          ...
          this.userphone = this.list.mobile;
          this.sheng = this.list.province;
          this.shi = this.list.city;
          this.qu = this.list.area;
          this.address = this.sheng + this.shi + this.qu;
          this.address_street = this.list.address;
        }).catch((err) => {
          console.log(err);
        })
      },
      upd_address() {
        console.log(this.Province, 11)
        if (this.Province == false || this.City == false || this.District == false) {
          console.log(this.Province, this.City, this.District, 11)
          this.sheng = this.sheng;
          this.shi = this.shi;
          this.qu = this.qu;
          console.log(this.sheng, this.shi, this.qu, 22)
        } else {
          this.sheng = '';
          this.shi = '';
          this.qu = '';
